...
struct video_channel::impl final
{
  ...
  public:
    ...
    {
        return lock(format_desc_mutex_, [&] { return format_desc_; });
    }
    ...
    {
        return lock(channel_layout_mutex_, [&] { return channel_layout_; });
    }
    ...
    std::shared_ptr<void> add_tick_listener(std::function<void()> listener)
    {
        return lock(tick_listeners_mutex_, [&] {
            ...
            return std::shared_ptr<void>(
                nullptr, [=](void*) { lock(tick_listeners_mutex_, [&] { tick_listeners_.erase(tick_listener_id); }); });
        });
    }
...
void                       core::video_channel::audio_channel_layout(const core::audio_channel_layout& channel_layout)
{
    impl_->audio_channel_layout(channel_layout);
}
