                       ...
                       const spl::shared_ptr<media_info_repository>& media_info_repo)
{
    ...
    auto writeTimeStr =
        boost::posix_time::to_iso_string(boost::posix_time::from_time_t(boost::filesystem::last_write_time(path)));
    ...
    return std::wstring() + L"\"" + str + +L"\" " + media_info->clip_type + +L" " + sizeStr + +L" " + writeTimeWStr +
           +L" " + boost::lexical_cast<std::wstring>(media_info->duration) + +L" " +
           boost::lexical_cast<std::wstring>(media_info->time_base.numerator()) + L"/" +
           boost::lexical_cast<std::wstring>(media_info->time_base.denominator()) + L"\r\n";
}
...
std::wstring ListMedia(const spl::shared_ptr<media_info_repository>& media_info_repo,
                       const std::wstring&                           sub_directory = L"")
{
    std::wstringstream replyString;
    for (boost::filesystem::recursive_directory_iterator itr(get_sub_directory(env::media_folder(), sub_directory)),
         end;
         itr != end;
         ++itr)
                           ...
                           const std::wstring&                                sub_directory = L"")
{
    ...
    for (boost::filesystem::recursive_directory_iterator itr(get_sub_directory(env::template_folder(), sub_directory)),
         end;
         itr != end;
         ++itr) {
        if (boost::filesystem::is_regular_file(itr->path()) &&
            cg_registry->is_cg_extension(itr->path().extension().wstring())) {
            auto relativePath = get_relative_without_extension(itr->path(), env::template_folder());

            auto writeTimeStr = boost::posix_time::to_iso_string(
                boost::posix_time::from_time_t(boost::filesystem::last_write_time(itr->path())));
            writeTimeStr.erase(
                std::remove_if(writeTimeStr.begin(), writeTimeStr.end(), [](char c) { return std::isdigit(c) == 0; }),
                writeTimeStr.end());
            auto writeTimeWStr = std::wstring(writeTimeStr.begin(), writeTimeStr.end());

            auto sizeStr = boost::lexical_cast<std::string>(boost::filesystem::file_size(itr->path()));
            sizeStr.erase(std::remove_if(sizeStr.begin(), sizeStr.end(), [](char c) { return std::isdigit(c) == 0; }),
                          sizeStr.end());
            ...
            replyString << L"\"" << str << L"\" " << sizeWStr << L" " << writeTimeWStr << L" " << template_type
                        << L"\r\n";
        }
    }
...
void loadbg_describer(core::help_sink& sink, const core::help_repository& repository)
{
    ...
    sink.para()
        ->code(L"clip")
        ->text(L" will be parsed by available registered producer factories. ")
    ...
    sink.para()
        ...
        ->code(L"_a")
        ->text(L" is found this file will be used as key for the main ")
        ->code(L"clip")
        ->text(L".");
    sink.para()->code(L"loop")->text(L" will cause the clip to loop.");
    sink.para()->text(L"When playing and looping the clip will start at ")->code(L"frame")->text(L".");
    sink.para()->text(L"When playing and loop the clip will end after ")->code(L"frames")->text(L" number of frames.");
    sink.para()
        ->code(L"auto")
        ->text(L" will cause the clip to automatically start when foreground clip has ended (without play). ")
    ...
    sink.para()
        ->text(L"See ")
        ->see(L"Animation Types")
        ->text(L" for supported values for ")
        ->code(L"tween")
        ->text(L".");
    sink.para()
        ->text(L"See ")
        ->url(L"http://libav.org/libavfilter.html")
        ->text(L" for supported values for the ")
        ->code(L"filter")
        ->text(L" command.");
}
...
void play_describer(core::help_sink& sink, const core::help_repository& repository)
{
    ...
    sink.para()
        ->text(L"Moves clip from background to foreground and starts playing it. If a transition (see ")
        ->see(L"LOADBG")
        ->text(L") is prepared, it will be executed.");
    sink.para()
        ->text(L"If additional parameters (see ")
        ->see(L"LOADBG")
...
void pause_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Pauses playback of the foreground clip on the specified layer. The ")
        ->see(L"RESUME")
...
void stop_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Removes the foreground clip of the specified layer.");
    sink.para()->text(L"Examples:");
...
void call_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Calls method on the specified producer with the provided ")->code(L"param")->text(L" string.");
    sink.para()->text(L"Examples:");
...
void swap_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->text(L"By specifying ")
        ->code(L"TRANSFORMS")
        ->text(L" the transformations of the layers are swapped as well.");
...
void add_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> ADD 1-700 FILE filename.mov SEPARATE_KEY\n"
                 L">> REMOVE 1-700",
                 L"overriding the consumer index to easier remove later.");
    sink.para()->text(
        L"The streaming consumer is an implementation of the ffmpeg_consumer and supports many of the same arguments:");
    sink.example(L">> ADD 1 STREAM udp://localhost:5004 -vcodec libx264 -tune zerolatency -preset ultrafast -crf 25 "
                 L"-format mpegts -vf scale=240:180");
}
...
void remove_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Removes an existing consumer from ")
        ->code(L"video_channel")
        ->text(L". If ")
        ->code(L"consumer_index")
        ->text(L" is given, the consumer will be removed via its id. If ")
        ->code(L"parameters")
        ->text(L" are given instead, the consumer matching those parameters will be removed.");
    sink.para()->text(L"Examples:");
...
void print_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(
        L">> PRINT 1",
        L"will produce a PNG image with the current date and time as the filename for example 20130620T192220.png");
}
...
void data_list_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"if the optional ")
        ->code(L"sub_directory")
...
void cg_add_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(
        L"CG [video_channel:int]{-[layer:int]|-9999} ADD [cg_layer:int] [template:string] [play-on-load:0,1] {[data]}");
    sink.para()
        ->text(L"Prepares a template for displaying. It won't show until you call ")
        ->see(L"CG PLAY")
        ->text(L" (unless you supply the play-on-load flag, 1 for true). Data is either inline XML or a reference to a "
               L"saved dataset.");
    sink.para()->text(L"Examples:");
...
void cg_stop_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Stops and removes the template from the specified layer. This is different from ")
        ->code(L"REMOVE")
...
void cg_update_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Sends new data to the template on specified layer. Data is either inline XML or a reference to "
                      L"a saved dataset.");
}
...
void cg_info_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"If ")
        ->code(L"cg_layer")
        ->text(L" is not given, information about the template host is given instead.");
}
...
void mixer_keyer_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Replaces layer ")
        ->code(L"n+1")
        ->text(L"'s alpha with the ")
        ->code(L"R")
        ->text(L" (red) channel of layer ")
        ->code(L"n")
        ->text(L", and hides the RGB channels of layer ")
        ->code(L"n")
    ...
    sink.example(L">> MIXER 1-0 KEYER\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 1",
...
void mixer_chroma_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} CHROMA {[enable:0,1] {[target_hue:float] "
                L"[hue_width:float] [min_saturation:float] [min_brightness:float] [softness:float] "
                L"[spill_suppress:float] [spill_suppress_saturation:float] [show_mask:0,1]}}" +
                ANIMATION_SYNTAX);
    sink.para()->text(L"Enables or disables chroma keying on the specified video layer. Giving no parameters returns "
                      L"the current chroma settings.");
    sink.para()->text(L"The chroma keying is done in the HSB/HSV color space.");
    sink.para()->text(L"Parameters:");
    sink.definitions()
        ->item(L"enable",
               L"0 to disable chroma keying on layer. The rest of the parameters should not be given when disabling.")
        ->item(L"target_hue", L"The hue in degrees between 0-360 where the center of the hue window will open up.")
        ->item(L"hue_width",
               L"The width of the hue window within 0.0-1.0 where 1.0 means 100% of 360 degrees around target_hue.")
        ->item(L"min_saturation",
               L"The minimum saturation within 0.0-1.0 required for a color to be within the chroma window.")
        ->item(L"min_brightness",
               L"The minimum brightness within 0.0-1.0 required for a color to be within the chroma window.")
        ->item(L"softness", L"The softness of the chroma keying window.")
        ->item(L"spill_suppress",
               L"How much to suppress spill by within 0.0-180.0. It works by taking all hue values within +- this "
               L"value from target_hue and clamps it to either target_hue - this value or target_hue + this value "
               L"depending on which side it is closest to.")
        ->item(L"spill_suppress_saturation",
               L"Controls how much saturation should be kept on colors affected by spill_suppress within 0.0-1.0. Full "
               L"saturation may not always be desirable to be kept on suppressed colors.")
        ->item(L"show_mask", L"If enabled, only shows the mask. Useful while editing the chroma key settings.");
    sink.example(L">> MIXER 1-1 CHROMA 1 120 0.1 0 0 0.1 0.1 0.7 0",
                 L"for enabling chroma keying centered around a hue of 120 degrees (green) and with a 10% hue width");
    sink.example(L">> MIXER 1-1 CHROMA 0", L"for disabling chroma keying");
    sink.example(L">> MIXER 1-1 CHROMA 0\n"
                 L"<< 202 MIXER OK\n"
                 L"<< 1 120 0.1 0 0 0.1 0 1 0",
                 L"for getting the current chroma key mode");
    sink.para()->text(L"Deprecated legacy syntax:");
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} CHROMA {[color:none,green,blue] {[threshold:float] "
                L"[softness:float] [spill:float]}}" +
                ANIMATION_SYNTAX);
...
void mixer_blend_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Every layer can be set to use a different ")
        ->url(L"http://en.wikipedia.org/wiki/Blend_modes", L"blend mode")
        ->text(L" than the default ")
        ->code(L"normal")
        ->text(L" mode, similar to applications like Photoshop. ")
        ->text(L"Some common uses are to use ")
        ->code(L"screen")
        ->text(L" to make all the black image data become transparent, ")
        ->text(L"or to use ")
        ->code(L"add")
        ->text(L" to selectively lighten highlights.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-1 BLEND OVERLAY");
    sink.example(L">> MIXER 1-1 BLEND\n"
                 L"<< 201 MIXER OK\n"
                 L"<< SCREEN",
                 L"for getting the current blend mode");
...
void mixer_opacity_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1-0 OPACITY\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.5",
...
void mixer_brightness_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1-0 BRIGHTNESS\n"
                 L"<< 201 MIXER OK\n"
                 L"0.5",
...
void mixer_saturation_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1-0 SATURATION\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.5",
...
void mixer_contrast_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1-0 CONTRAST\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.5",
...
void mixer_levels_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} LEVELS {[min-input:float] [max-input:float] [gamma:float] "
                L"[min-output:float] [max-output:float]" +
                ANIMATION_SYNTAX);
    sink.para()->text(
        L"Adjusts the video levels of a layer. If no arguments are given the current levels are returned.");
    sink.definitions()
        ...
        ->item(L"min-output, max-output",
               L"Defines the output range (between 0 and 1) to scale the accepted input RGB values to.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-10 LEVELS 0.0627 0.922 1 0 1 25 easeinsine",
                 L"for stretching 16-235 video to 0-255 video");
    sink.example(L">> MIXER 1-10 LEVELS 0 1 1 0.0627 0.922 25 easeinsine",
                 L"for compressing 0-255 video to 16-235 video");
    sink.example(L">> MIXER 1-10 LEVELS 0 1 0.5 0 1 25 easeinsine", L"for adjusting the gamma to 0.5");
    sink.example(L">> MIXER 1-10 LEVELS\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0 1 0.5 0 1",
...
void mixer_fill_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(
        L"MIXER [video_channel:int]{-[layer:int]|-0} FILL {[x:float] [y:float] [x-scale:float] [y-scale:float]" +
        ANIMATION_SYNTAX);
    sink.para()
        ...
        ->text(L"which means left edge is at coordinate 0, top edge at coordinate 0, width full size = 1, heigh full "
               L"size = 1.");
    sink.para()
        ->text(L"If you want to crop the picture on the left side (for wipe left to right) ")
        ->text(L"You set the left edge to full right => 1 and the width to 0. So this give you the start-coordinates "
               L"of 1 0 0 1.");
    ...
    sink.para()
        ...
        ->text(L"if you want to do a smaller window. If, for instance you want to have a window of half the size of "
               L"your screen, ")
        ->text(L"you set with and height to 0.5. If you want to center it you set left and top edge to 0.25 so you "
               L"will get the arguments 0.25 0.25 0.5 0.5");
    sink.definitions()
        ->item(L"x",
               L"The new x position, 0 = left edge of monitor, 0.5 = middle of monitor, 1.0 = right edge of monitor. "
               L"Higher and lower values allowed.")
        ->item(L"y",
               L"The new y position, 0 = top edge of monitor, 0.5 = middle of monitor, 1.0 = bottom edge of monitor. "
               L"Higher and lower values allowed.")
        ->item(L"x-scale",
               L"The new x scale, 1 = 1x the screen width, 0.5 = half the screen width. Higher and lower values "
               L"allowed. Negative values flips the layer.")
        ->item(L"y-scale",
               L"The new y scale, 1 = 1x the screen height, 0.5 = half the screen height. Higher and lower values "
               L"allowed. Negative values flips the layer.");
    sink.para()
        ->text(L"The positioning and scaling is done around the anchor point set by ")
        ->see(L"MIXER ANCHOR")
        ->text(L".");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-0 FILL 0.25 0.25 0.5 0.5 25 easeinsine");
    sink.example(L">> MIXER 1-0 FILL\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.25 0.25 0.5 0.5",
...
void mixer_clip_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} CLIP {[x:float] [y:float] [width:float] [height:float]" +
                ANIMATION_SYNTAX);
    sink.para()
        ->text(
            L"Defines the rectangular viewport where a layer is rendered thru on the screen without being affected by ")
        ->see(L"MIXER FILL")
        ->text(L", ")
        ->see(L"MIXER ROTATION")
        ->text(L" and ")
        ->see(L"MIXER PERSPECTIVE")
        ->text(L". See ")
        ->see(L"MIXER CROP")
        ->text(L" if you want to crop the layer before transforming it.");
    sink.definitions()
        ->item(L"x",
               L"The new x position, 0 = left edge of monitor, 0.5 = middle of monitor, 1.0 = right edge of monitor. "
               L"Higher and lower values allowed.")
        ->item(L"y",
               ...
               L"Higher and lower values allowed.")
        ->item(L"width",
               L"The new width, 1 = 1x the screen width, 0.5 = half the screen width. Higher and lower values allowed. "
               L"Negative values flips the layer.")
        ->item(L"height",
               L"The new height, 1 = 1x the screen height, 0.5 = half the screen height. Higher and lower values "
               L"allowed. Negative values flips the layer.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-0 CLIP 0.25 0.25 0.5 0.5 25 easeinsine");
    sink.example(L">> MIXER 1-0 CLIP\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.25 0.25 0.5 0.5",
...
void mixer_anchor_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(
        L"Changes the anchor point of the specified layer, or returns the current values if no arguments are given.");
    sink.para()
        ->text(L"The anchor point is around which ")
        ->see(L"MIXER FILL")
        ->text(L" and ")
        ->see(L"MIXER ROTATION")
    ...
    sink.definitions()
        ->item(L"x",
               L"The x anchor point, 0 = left edge of layer, 0.5 = middle of layer, 1.0 = right edge of layer. Higher "
               L"and lower values allowed.")
        ->item(L"y",
               L"The y anchor point, 0 = top edge of layer, 0.5 = middle of layer, 1.0 = bottom edge of layer. Higher "
               L"and lower values allowed.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-10 ANCHOR 0.5 0.6 25 easeinsine", L"sets the anchor point");
    sink.example(L">> MIXER 1-10 ANCHOR\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.5 0.6",
...
void mixer_crop_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} CROP {[left-edge:float] [top-edge:float] "
                L"[right-edge:float] [bottom-edge:float]" +
                ANIMATION_SYNTAX);
    sink.para()
        ...
        ->text(L". See ")
        ->see(L"MIXER CLIP")
        ->text(L" if you want to change the viewport relative to the screen instead.");
    sink.definitions()
        ...
        ->item(L"bottom-edge",
               L"A value between 1 and 0 defining how far into the layer to crop from the bottom edge.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-0 CROP 0.25 0.25 0.75 0.75 25 easeinsine", L"leaving a 25% crop around the edges");
    sink.example(L">> MIXER 1-0 CROP\n"
                 L"<< 201 MIXER OK\n"
...
void mixer_rotation_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->text(L"around the point specified by ")
        ->see(L"MIXER ANCHOR")
        ->text(L".");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-0 ROTATION 45 25 easeinsine");
    sink.example(L">> MIXER 1-0 ROTATION\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 45",
...
void mixer_perspective_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"MIXER [video_channel:int]{-[layer:int]|-0} PERSPECTIVE {[top-left-x:float] [top-left-y:float] "
                L"[top-right-x:float] [top-right-y:float] [bottom-right-x:float] [bottom-right-y:float] "
                L"[bottom-left-x:float] [bottom-left-y:float]" +
                ANIMATION_SYNTAX);
    sink.para()->text(L"Perspective transforms (corner pins or distorts if you will) a layer.");
    sink.definitions()
    ...
    sink.example(L">> MIXER 1-10 PERSPECTIVE\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.4 0.4 0.6 0.4 1 1 0 1",
...
void mixer_mipmap_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1-10 MIPMAP\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 1",
...
void mixer_volume_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Changes the volume of the specified layer. The 1.0 is the original volume, which can be "
                      L"attenuated or amplified.");
    ...
    sink.example(L">> MIXER 1-0 VOLUME\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 0.8",
...
void mixer_straight_alpha_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.example(L">> MIXER 1 STRAIGHT_ALPHA_OUTPUT\n"
                 L"<< 201 MIXER OK\n"
                 L"<< 1");
}
...
void mixer_grid_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->code(L"resolution")
        ->text(L" equals 2 then a 2x2 grid of layers will be created starting from layer 1.");
    sink.para()->text(L"Examples:");
...
void mixer_commit_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Commits all deferred mixer transforms on the specified channel. This ensures that all "
                      L"animations start at the same exact frame.");
    sink.para()->text(L"Examples:");
    sink.example(L">> MIXER 1-1 FILL 0 0 0.5 0.5 25 DEFER\n"
...
void channel_grid_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"The element ")
        ->code(L"<channel-grid>true</channel-grid>")
        ->text(L" must be present in ")
        ->code(L"casparcg.config")
        ->text(L" for this to work correctly.");
}
...
void thumbnail_retrieve_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Examples:");
    sink.example(L">> THUMBNAIL RETRIEVE foo/bar\n"
...
void cls_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Lists media files in the ")
        ->code(L"media")
        ->text(L" folder. Use the command ")
        ->see(L"INFO PATHS")
        ->text(L" to get the path to the ")
        ->code(L"media")
        ->text(L" folder.");
    sink.para()
        ->text(L"if the optional ")
...
void fls_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"FLS");
    sink.para()
        ->text(L"Lists all font files in the ")
        ->code(L"fonts")
        ->text(L" folder. Use the command ")
        ->see(L"INFO PATHS")
        ->text(L" to get the path to the ")
...
std::wstring fls_command(command_context& ctx)
{
    ...
    for (auto& font : core::text::list_fonts())
        replyString << L"\"" << font.first << L"\" \"" << get_relative(font.second, env::font_folder()).wstring()
                    << L"\"\r\n";
...
void tls_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Lists template files in the ")
        ->code(L"templates")
        ->text(L" folder. Use the command ")
        ->see(L"INFO PATHS")
        ->text(L" to get the path to the ")
...
void version_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()->text(L"Examples:");
    sink.example(L">> VERSION\n"
                 L"<< 201 VERSION OK\n"
                 L"<< 2.1.0.f207a33 STABLE");
    sink.example(L">> VERSION SERVER\n"
                 L"<< 201 VERSION OK\n"
                 L"<< 2.1.0.f207a33 STABLE");
    sink.example(L">> VERSION FLASH\n"
                 L"<< 201 VERSION OK\n"
                 L"<< 11.8.800.94");
    sink.example(L">> VERSION TEMPLATEHOST\n"
                 L"<< 201 VERSION OK\n"
                 L"<< unknown");
    sink.example(L">> VERSION CEF\n"
                 L"<< 201 VERSION OK\n"
                 L"<< 3.1750.1805");
}
...
void gl_gc_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"GL GC");
    sink.para()
        ->text(L"Releases all the pooled OpenGL resources. ")
        ->strong(L"May cause a pause on all video channels.");
}
...
struct max_width_sink : public core::help_sink
{
    ...
    void begin_item(const std::wstring& name) override { max_width = std::max(name.length(), max_width); };
};
...
struct short_description_sink : public core::help_sink
{
    ...
    short_description_sink(std::size_t width, std::wstringstream& out)
        : width(width)
        , out(out)
    {
    }

    void begin_item(const std::wstring& name) override { out << std::left << std::setw(width + 1) << name; };

    void short_description(const std::wstring& short_description) override { out << short_description << L"\r\n"; };
};
...
struct simple_paragraph_builder : core::paragraph_builder
{
    ...
    simple_paragraph_builder(std::wstringstream& out)
        : commit_to(out)
    {
    }
    ~simple_paragraph_builder() { commit_to << core::wordwrap(out.str(), WIDTH) << L"\n"; }
    ...
    spl::shared_ptr<paragraph_builder> strong(std::wstring item) override
    {
        return text(L"*" + std::move(item) + L"*");
    }
...
struct simple_definition_list_builder : core::definition_list_builder
{
    ...
    simple_definition_list_builder(std::wstringstream& out)
        : out(out)
    {
    }
    ~simple_definition_list_builder() { out << L"\n"; }
...
struct long_description_sink : public core::help_sink
{
    ...
    long_description_sink(std::wstringstream& out)
        : out(out)
    {
    }
    ...
    spl::shared_ptr<core::paragraph_builder> para() override { return spl::make_shared<simple_paragraph_builder>(out); }
  ...
  private:
    void begin_item(const std::wstring& name) override { out << name << L"\n\n"; };
};
...
std::wstring
create_help_details(const std::wstring& help_command, const command_context& ctx, std::set<std::wstring> tags)
{
    std::wstringstream result;
...
void bye_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->text(L"on the machine Caspar is running on then this will achieve the same as the ")
        ->see(L"KILL")
        ->text(L" command.");
}
...
void restart_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->text(L"Intended for use in combination with ")
        ->code(L"casparcg_auto_restart.bat");
}
...
void req_describer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ...
        ->code(L"RES request_id")
        ->text(L" to the response, in order to see what asynchronous response matches what request.");
    sink.para()->text(L"Examples:");
