  ...
  public:
        ...
        , channel_layout_(get_adjusted_channel_layout(channel_layout))
    {
        ...
        auto display_mode = get_display_mode(
            input_, in_format_desc.format, bmdFormat8BitYUV, bmdVideoInputFlagDefault, will_attempt_dma);
        ...
        try {
            ...
            monitor_subject_ << core::monitor::message("/file/video/width") % video->GetWidth()
                             ...
                             << core::monitor::message("/file/video/field") %
                                    u8(!video_frame->interlaced_frame
                                           ? "progressive"
                                           : (video_frame->top_field_first ? "upper" : "lower"))
            ...
            if (audio && SUCCEEDED(audio->GetBytes(&audio_bytes)) && audio_bytes) {
                ...
                audio_buffer = std::make_shared<core::mutable_audio_buffer>(
                    audio_data, audio_data + sample_frame_count * channel_layout_.num_channels);
            } else
                audio_buffer = std::make_shared<core::mutable_audio_buffer>(
    ...
    {
        return model_name_ + L" [" + boost::lexical_cast<std::wstring>(device_index_) + L"|" + in_format_desc_.name +
               L"]";
    }
...
void describe_producer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.syntax(L"DECKLINK [device:int],DEVICE [device:int] {FILTER [filter:string]} {LENGTH [length:int]} {FORMAT "
                L"[format:string]} {CHANNEL_LAYOUT [channel_layout:string]}");
    sink.para()->text(L"Allows video sources to be input from BlackMagic Design cards. Parameters:");
    sink.definitions()
        ->item(L"device",
               L"The decklink device to stream the input from. See the Blackmagic control panel for the order of "
               L"devices in your system.")
        ...
        ->item(L"format",
               L"Specifies what video format to expect on the incoming SDI/HDMI signal. If not specified the video "
    ...
    sink.example(
        ...
        L"Play using decklink device 2 expecting the video signal to have the same video format as the channel.");
    sink.example(L">> PLAY 1-10 DECKLINK DEVICE 2 FORMAT PAL FILTER yadif=1:-1",
                 L"Play using decklink device 2 expecting the video signal to be in PAL and deinterlace it.");
    sink.example(L">> PLAY 1-10 DECKLINK DEVICE 2 LENGTH 1000",
                 L"Play using decklink device 2 but only produce 1000 frames.");
    sink.example(L">> PLAY 1-10 DECKLINK DEVICE 2 CHANNEL_LAYOUT smpte",
                 L"Play using decklink device 2 and expect smpte surround sound.");
}
